// =============================================
// PLANAC ERP - Caixa Econômica Federal Webhook Service
// Processamento de webhooks da Caixa
// =============================================
...
export interface CaixaWebhookPayload {
  /** Tipo do webhook: boleto ou pix */
  tipo: 'boleto' | 'pix';
  /** Dados do boleto (se tipo = boleto) */
  boleto?: CaixaWebhookBoleto;
...
/**
 * Processa webhook recebido da Caixa
 */
export function processWebhook(payload: CaixaWebhookPayload): WebhookProcessResult[] {
 ...
 */
function processBoletoWebhook(boleto: CaixaWebhookBoleto): WebhookProcessResult {
 ...
 */
function processPixWebhook(pix: CaixaWebhookPix): WebhookProcessResult {
  return {
    success: true,
    tipo: 'pix',
    idExterno: pix.txid,
...
export function getSituacaoDescricao(codigoSituacao: string): string {
  return CAIXA_SITUACOES[codigoSituacao] || `Situação ${codigoSituacao}`;
}
