// ============================================
// PLANAC ERP - Rotas de Integração Caixa
...
const caixa = new Hono<{ Bindings: Bindings; Variables: Variables }>();
...
// ==========================================
// WEBHOOK - Recebe atualizações da Caixa
// ==========================================

/**
 * POST /caixa/webhook/:contaId
 * 
 * Endpoint para receber webhooks da Caixa.
 */
caixa.post('/webhook/:contaId', async (c) => {
  ...
  try {
    ...
    `).bind(contaId, CAIXA_CODIGO_BANCO).first<{
      id: string;
      empresa_id: string;
    ...
    if (!conta) {
      console.error(`[Caixa Webhook] Conta não encontrada: ${contaId}`);
      return c.json({ success: false, error: 'Conta não encontrada' }, 404);
    }
    
    if (!conta.webhook_ativo) {
      console.warn(`[Caixa Webhook] Webhook inativo para conta: ${contaId}`);
    ...
    if (conta.webhook_symmetric_key && assinatura) {
      ...
      if (!isValid) {
        console.error(`[Caixa Webhook] Assinatura inválida`);
    ...
    if (existingWebhook) {
      console.log(`[Caixa Webhook] Webhook duplicado ignorado: ${payloadHash}`);
    ...
    for (const result of results) {
      ...
      if (result.tipo === 'boleto') {
        ...
        if (!titulo) {
          console.warn(`[Caixa Webhook] Título não encontrado: ${result.dados.nossoNumero}`);
          continue;
        }
    ...
    console.log(`[Caixa Webhook] Processado em ${processingTime}ms`);
    ...
    console.error(`[Caixa Webhook] Erro:`, error);
    return c.json({
      success: false,
 ...
 * POST /caixa/boletos
 * 
 * Registra um novo boleto na Caixa via API
 */
caixa.post('/boletos', requirePermission('financeiro', 'criar'), async (c) => {
  ...
  `).bind(data.conta_id, usuario.empresa_id, CAIXA_CODIGO_BANCO).first<{
    id: string;
    empresa_id: string;
  ...
  if (!conta) {
    return c.json({ success: false, error: 'Conta Caixa não encontrada' }, 404);
  }
  
  if (!conta.api_client_id || !conta.api_client_secret_encrypted) {
    return c.json({ success: false, error: 'Credenciais Caixa não configuradas' }, 400);
  }
  
  // Mapear dados para formato Caixa
  const boletoCaixa = mapErpToCaixa({
  ...
  // Validar dados
  const errosValidacao = validarBoleto(boletoCaixa);
  ...
  // Criar serviço Caixa
  const caixaService = createCaixaService({
    CAIXA_CLIENT_ID: conta.api_client_id,
    CAIXA_CLIENT_SECRET: conta.api_client_secret_encrypted, // TODO: decriptar
    CAIXA_AMBIENTE: conta.ambiente as 'homologacao' | 'producao',
    CAIXA_CODIGO_BENEFICIARIO: conta.codigo_beneficiario,
    CAIXA_CNPJ_BENEFICIARIO: '', // TODO: buscar da empresa
  });
  
  try {
    // Registrar boleto
    const response = await caixaService.boleto.registrar(boletoCaixa);
    ...
    `).bind(
      ...
      tituloId,
      'registro_api',
      'Boleto registrado via API Caixa'
    ).run();
...
/**
 * GET /caixa/boletos/:id
 * 
 * Consulta um boleto na Caixa via API
 */
caixa.get('/boletos/:id', requirePermission('financeiro', 'visualizar'), async (c) => {
  const usuario = c.get('usuario');
  ...
  `).bind(id, usuario.empresa_id, CAIXA_CODIGO_BANCO).first<{
    id: string;
    nosso_numero: string;
    api_client_id: string;
    api_client_secret_encrypted: string;
  ...
  // Criar serviço Caixa
  const caixaService = createCaixaService({
    CAIXA_CLIENT_ID: titulo.api_client_id,
    CAIXA_CLIENT_SECRET: titulo.api_client_secret_encrypted,
    CAIXA_AMBIENTE: titulo.ambiente as 'homologacao' | 'producao',
    CAIXA_CODIGO_BENEFICIARIO: titulo.codigo_beneficiario,
    CAIXA_CNPJ_BENEFICIARIO: '',
  });
  
  try {
    const response = await caixaService.boleto.consultar(titulo.nosso_numero);
...
/**
 * POST /caixa/testar-conexao
 * 
 * Testa a conexão com a API da Caixa
 */
caixa.post('/testar-conexao', requirePermission('configuracoes', 'visualizar'), async (c) => {
  const usuario = c.get('usuario');
  const body = await c.req.json();
  ...
  const conta = await c.env.DB.prepare(`
    ...
    JOIN boleto_bancos bb ON bc.banco_id = bb.id
    WHERE bc.id = ? AND bc.empresa_id = ? AND bb.codigo = ?
  `).bind(conta_id, usuario.empresa_id, CAIXA_CODIGO_BANCO).first<{
    api_client_id: string;
    api_client_secret_encrypted: string;
    ambiente: string;
    codigo_beneficiario: string;
  ...
  const caixaService = createCaixaService({
    CAIXA_CLIENT_ID: conta.api_client_id,
    CAIXA_CLIENT_SECRET: conta.api_client_secret_encrypted,
    CAIXA_AMBIENTE: conta.ambiente as 'homologacao' | 'producao',
    CAIXA_CODIGO_BENEFICIARIO: conta.codigo_beneficiario,
    CAIXA_CNPJ_BENEFICIARIO: '',
  });
  
  const result = await caixaService.utils.testConnection();
